...
class Home {
  ...
  constructor(page: Page) {
    ...
    this.user = this.page.locator("a", {
      has: this.page.locator("//i[@class='fa fa-user']"),
    });
    ...
    this.addToCartBtn = this.page.locator(
      "//a[text()[normalize-space()='Add to cart']]"
    );

    this.categories = this.page.locator("#accordian .panel");
    this.recommendedItems = this.page.locator(".recommended_items");
    this.recommendedItemsTitle =
      this.recommendedItems.getByText("recommended items");
    this.recommendedProducts = this.recommendedItems.locator(
      ".active .single-products"
    );
  }
  ...
  async clickCategory(category: string, subcategory: string) {
    const categoryLoc = this.categories
      .nth(this.categoriesMap[category])
      .locator(`a[href="#${category}"]`);
    await categoryLoc.waitFor({ state: "attached" });
    await categoryLoc.click();

    const subCategoryLoc = this.page.locator(`#${category} a`, {
      hasText: `${subcategory}`,
    });
    await subCategoryLoc.waitFor({ state: "attached" });
