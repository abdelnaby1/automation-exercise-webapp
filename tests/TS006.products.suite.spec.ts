...
test.describe("Products Suite", () => {
  ...
  test("TC009 - all products should be visible successfully", async ({
    page,
    prodcutsPage,
    request,
  }) => {
  ...
  test("TC010 - it should show product details on clicking on view product", async ({
    page,
    prodcutsPage,
    productDetailsPage,
  }) => {
  ...
  test("TC011 - it should review a product successfully", async ({
    page,
    prodcutsPage,
    productDetailsPage,
  }) => {
    ...
    await productDetailsPage.reviewProduct(
      loggedInUser.firstName,
      loggedInUser.email,
      "amazing product"
    );
    expect(await productDetailsPage.getReviewMsg()).toContain(
